...
# 
# Further in code, XYZ can be turned into any isotropic (XXX, XXZ, etc.) form
# by defining equal X Y and Z arrays of couplings
...
def log_like(J_arr, N_spins, dataset, beta):
    ...
    $$L = -\sum_{\{\sigma_m\}} \log \left( |\langle \sigma_m |U_m \frac{e^{-\beta H(J)}}{Z(J)} U_m^+|\sigma_m\rangle|^2 \right)$$
    where $\sigma_m$ - results of the projective measurements in the basis $m$
    """
...
def optim_ll(N_spins, x0, batch_ds, beta, J_arr, ll):
    ...
    ------
    True J_xx: {tr_J_xx}
    Recovered J_xx: {rec_J_xx}
    rmse: {np.sqrt(mse(tr_J_xx, rec_J_xx))}
    ------
    True J_yy: {tr_J_yy}
    Recovered J_yy: {rec_J_yy}
    rmse: {np.sqrt(mse(tr_J_yy, rec_J_yy))}
    ------
    True J_xx: {tr_J_zz}
    Recovered J_zz: {rec_J_zz}
    rmse: {np.sqrt(mse(tr_J_zz, rec_J_zz))}
    ------
    True h_x: {tr_h_x}
    Recovered h_x: {rec_h_x}
    rmse: {np.sqrt(mse(tr_h_x, rec_h_x))}
    ------
    Total RMSE: {err}
    ------
    True log like maximum: {ll}
    Optimized maximum: {res['fun']}
    ------
    Optimization time, s: {t2 - t1}
    '''
    )
